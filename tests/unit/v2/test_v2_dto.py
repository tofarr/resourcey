...
def test_timestamp_convention_gets_create_and_update_defaults():
    ...
    # An explicit DtoField is honoured verbatim: the convention must not add a
    # factory where the declaration already supplied one.
    ...
    class BareTimestamps(DTO):
        id: int
        created_at: datetime
        updated_at: datetime

    fields = BareTimestamps.get_fields()
...
def test_uuid_id_gets_a_server_side_factory():
    # A bare ``id: UUID`` is generated by the application, not the database.
    fields = Node.get_fields()
    assert fields["id"].default_factory_for_create is uuid4
    assert fields["id"].in_create_request is False
    assert isinstance(Node.get_default("id"), UUID)
    assert Node.get_default("id") != Node.get_default("id")


def test_uuid_id_factory_is_used_on_create_but_absent_from_requests():
    # An omitted id resolves through ``new`` without a database round trip.
    instance = Node.new(label="n")
    assert isinstance(instance.id, UUID)
    models = Node.get_rest_models()
    assert "id" not in models.create_request.model_fields
    assert "id" not in models.update_request.model_fields


def test_uuid_custom_identifier_stays_client_supplied():
    class Node(DTO, id_field_name="node_id"):
        node_id: UUID
        label: str

    field = Node.get_fields()["node_id"]
    # A natural key: the client supplies it on create, and no factory is generated.
    assert field.in_create_request is True
    assert field.in_update_request is False
    assert field.has_default_for("create") is False
    assert Node.get_default("node_id") is MISSING
    assert Node.new(node_id=uuid4(), label="n").node_id is not MISSING


def test_non_uuid_id_gets_no_generated_factory():
    class IntNode(DTO):
        id: int
        label: str

    assert IntNode.get_fields()["id"].has_default_for("create") is False
    assert IntNode.get_default("id") is MISSING


# ---------------------------------------------------------------------------
# Conventions never override expressed intent (issue #94)
# ---------------------------------------------------------------------------


def test_explicit_dto_field_on_id_is_honoured_verbatim():
    class M(DTO):
        id: Annotated[UUID, DtoField(in_create_request=True, in_update_request=True)]
        label: str

    field = M.get_fields()["id"]
    # The convention would normally exclude id from create/update and add a
    # factory; an explicit DtoField wins outright.
    assert field.in_create_request is True
    assert field.in_update_request is True
    assert field.has_default_for("create") is False
    assert "id" in M.get_rest_models().create_request.model_fields
    assert "id" in M.get_rest_models().update_request.model_fields


def test_explicit_dto_field_on_timestamps_is_honoured_verbatim():
    class M(DTO):
        id: int
        created_at: Annotated[datetime, DtoField(in_create_request=True, in_read_response=False)]
        updated_at: Annotated[datetime, DtoField(default_for_create=None)]

    created = M.get_fields()["created_at"]
    assert created.in_create_request is True
    assert created.in_read_response is False
    assert created.has_default_for("create") is False
    assert "created_at" not in M.get_rest_models().read_response.model_fields

    updated = M.get_fields()["updated_at"]
    assert updated.default_for_create is None
    assert updated.has_default_for("update") is False


def test_timestamp_conventions_do_not_clobber_a_declared_default():
    class M(DTO):
        id: int
        updated_at: Annotated[datetime, DtoField(default_factory_for_create=utc_now)]
        created_at: Annotated[datetime, DtoField(default_factory_for_update=utc_now)]

    fields = M.get_fields()
    assert fields["updated_at"].default_factory_for_create is utc_now
    # An explicit update default is kept, not replaced by the created_at convention.
    assert fields["created_at"].default_factory_for_update is utc_now


# ---------------------------------------------------------------------------
# Contradictory defaults (issue #94)
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ("kwargs", "operation"),
    [
        ({"default_for_create": 1, "default_factory_for_create": uuid4}, "create"),
        ({"default_for_update": 1, "default_factory_for_update": uuid4}, "update"),
    ],
)
def test_declaring_a_value_and_a_factory_for_the_same_operation_raises(kwargs, operation):
    with pytest.raises(ValueError, match=f"both a value and a factory default for {operation}"):
        DtoField(**kwargs)


def test_a_value_and_a_factory_for_different_operations_is_allowed():
    field = DtoField(default_for_create=1, default_factory_for_update=uuid4)
    assert field.default_for_create == 1
    assert field.default_factory_for_update is uuid4


def test_contradictory_defaults_raise_at_declaration_time():
    with pytest.raises(ValueError, match="both a value and a factory default for create"):

        class Bad(DTO):
            id: int
            code: Annotated[
                str, DtoField(default_for_create="x", default_factory_for_create=lambda: "y")
            ]


def test_contradictory_defaults_via_a_bare_class_attribute_also_raise():
    # A DtoField as a class attribute is still resolved at subclass creation.
    with pytest.raises(ValueError, match="both a value and a factory default for update"):

        class Bad(DTO):
            id: int
            code: str = DtoField(default_for_update="x", default_factory_for_update=lambda: "y")


# ---------------------------------------------------------------------------
# Generated defaults never reach a response model (issue #94)
# ---------------------------------------------------------------------------


def test_a_generated_default_never_appears_on_any_response_model():
    class Stored(DTO):
        id: UUID
        token: Annotated[str, DtoField(default_factory_for_create=lambda: "generated")]
        created_at: datetime
        updated_at: datetime

    models = Stored.get_rest_models()
    for response in (
        ...
        # The generated value is not on the wire until the resource is served;
        # the response shapes carry no default at all, every field is required.
        for field in response.model_fields.values():
            assert field.is_required()
            assert field.default_factory is None


def test_timestamps_appear_in_all_response_shapes_and_no_request_shape():
    class Stored(DTO):
        id: int
        created_at: datetime
        updated_at: datetime
    ...
    for response in (
        ...
        assert "updated_at" in response.model_fields
    assert "created_at" not in models.create_request.model_fields
    assert "updated_at" not in models.create_request.model_fields
    assert "created_at" not in models.update_request.model_fields
    assert "updated_at" not in models.update_request.model_fields
