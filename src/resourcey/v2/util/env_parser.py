...
Vendored from the OpenHands Software Agent SDK (written by the same author)
...
JsonType = str | int | float | bool | dict | list | None | Missing
...
class BoolEnvParser(EnvParser):
    def from_env(self, key: str) -> bool | Missing:
...
class IntEnvParser(EnvParser):
    def from_env(self, key: str) -> int | Missing:
        if key not in os.environ:
            return MISSING
...
class FloatEnvParser(EnvParser):
    def from_env(self, key: str) -> float | Missing:
        if key not in os.environ:
            return MISSING
...
class StrEnvParser(EnvParser):
    def from_env(self, key: str) -> str | Missing:
        if key not in os.environ:
            return MISSING
...
class NoneEnvParser(EnvParser):
    def from_env(self, key: str) -> Missing | None:
        key = f"{key}_IS_NONE"
...
class ModelEnvParser(EnvParser):
    ...
    def from_env(self, key: str) -> dict | Missing:
        ...
        for field_name, parser in self.parsers.items():
            ...
            # infinite recursion
            has_possible_keys = next((k for k in os.environ if k.startswith(env_var_name)), False)
...
class ListEnvParser(EnvParser):
    ...
    def from_env(self, key: str) -> list | Missing:
        if key not in os.environ:
    ...
    def to_env(self, key: str, value: Any, output: IO):
        if len(value):
            ...
            try:
                ...
                self.item_parser.to_env(sub_key, _create_sample(self.item_type), sample_output)
    ...
    def from_env(self, key: str) -> JsonType:
        ...
        for parser in self.parsers.values():
            try:
                ...
                # This parser can't handle the env value; skip it
                continue
...
class DiscriminatedUnionEnvParser(EnvParser):
    ...
    def to_env(self, key: str, value: Any, output: IO):
        output.write(f"{key}_KIND={value.kind}\n")
