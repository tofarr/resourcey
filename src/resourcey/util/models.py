"""Polymorphism support: discriminated unions keyed by a `kind` field.
...
def get_handler_class_name(handler: SerializerFunctionWrapHandler) -> str:
    ...
    including tests that verify serialization behavior across the class hierarchy.
    """
...
class DiscriminatedUnionMixin(BaseModel):
    """Mixin that adds a `kind` discriminator to a pydantic model.

    Subclasses are serialized with a ``kind`` field equal to the class name;
    deserialization routes to the correct subclass based on that field. Abstract
    bases (those extending :class:`abc.ABC`) are never instantiated directly.
    """
    ...
    def get_serializable_type(cls) -> type:
        ...
        if len(subclasses) == 1:
            ...
            # rather than the abstract base when there is only ONE concrete subclass.
