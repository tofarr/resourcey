...
| All entities as resources | `users_and_permissions/auth_resources.py` | Every auth backing entity (IdP tokens, OAuth clients, API keys) is declared as a resource so the migration is autogenerated from `ResourceyBase.metadata` alone — but with `is_exposed() == False` so none appear in the REST API. |

## Resources

Every entity is a `SecuredSqlResource`, so a single resource manifest owns the
...
over REST; the auth backing entities are **internal** (`is_exposed() == False`)
...
All resources inherit from `SecuredSqlResource` (this example) instead of
...
│   └── auth_resources.py   # IdP/OAuth/ApiKey resources (internal; is_exposed=False)
├── migrations/
...
declared in `auth_resources.py` with `is_exposed() == False`. The result is a
